...
static xQueueHandle tdoaDataQueue;// TODO: Find a better check for whether the quad is flying
...
//thrust is thrust mapped/ TODO: Find a better check for whether the quad is flying for 65536 <==> 60 GRAMS!
...
void estimatorKalman(state_t *state, sensorData_t *sensors, control_t *control, const uint32_t tick)
{
 // If the client (via a parameter update) triggers an estimator reset:
  if (coreData.resetEstimation/* TODO: Find a better check for whether the quad is flyingation*/) { estimatorKalmanInit(); coreData.resetEstimation = false; }
...
//    thrustAccumulator *= CONTROL_TO_ACC;
 ...
 //   thrustAccumulator /= thrustAccumulatorCount;
    ...
    thrustAccumulator = 0.0;
    // TODO: Find a better check for whether the quad is flying
    // Assume that the flight begins when the thrust is large enough and for now we never stop "flying".
/*    if (thrustAccumulat/ TODO: Find a better check for whether the quad is flyingor > IN_FLIGHT_THRUST_THRESHOLD) {
 ...
 //    DEBUG_PRINT("x=%f \n",(double)coreData.S[KC_STATE_X]) ;
  ...
  heightMeasurement_t height;// TODO: Find a better check for whether the quad is flying
 ...
 //   DEBUG_PRINT("Hello kalman core dist\n");
    doneUpdate = true;
  }
  ...
  pos1.x = coreData.S[KC_STATE_X];
  pos1.y = coreData.S[KC_STATE_Y];
  pos1.z = coreData.S[KC_STATE_Z];
 // DEBUG_PRINT("x=%f \n",(double)coreData.S[KC_STATE_X]) ;
  /**
...
{
  ASSERT(isInit);
  DEBUG_PRINT("estimatorKalmanEnqueueTDOA");
...
{
  ASSERT(isInit);
  DEBUG_PRINT("estimatorKalmanEnqueuePosition");
...
{
  ASSERT(isInit);
  DEBUG_PRINT("estimatorKalmanEnqueuePose");
...
{
  ASSERT(isInit);
  //DEBUG_PRINT("estimatorKalmanEnqueueDistance \n");
...
bool estimatorKalmanEnqueueFlow(const flowMeasurement_t *flow)
{
  ...
  ASSERT(isInit);
  DEBUG_PRINT("estimatorKalmanEnqueueFlow");
...
bool estimatorKalmanEnqueueTOF(const tofMeasurement_t *tof)
{
  ...
  ASSERT(isInit);
  DEBUG_PRINT("estimatorKalmanEnqueueTOF");
